...
cherry-pick の際にコミットハッシュを指定していたのだけどブランチの差分でもできることを知った

```sh
git switch <cherry-pick先ブランチ>
git cherry-pick <cherry-pickしたい元ブランチ>..<cherry-pick先ブランチ>
```

こうすると元ブランチから最新コミットまでの差分を cherry-pick できる

ローカルおかしくなったときにブランチ rename して remote から取得して cherry-pick できそう

ていうかコミットハッシュで1個づつやってたのよな

コミットハッシュも以下のように範囲指定できた

```sh
git cherry-pick <cherry-pickしたい先頭のコミットハッシュ>..<cherry-pickしたい最新のコミットハッシュ>
```
